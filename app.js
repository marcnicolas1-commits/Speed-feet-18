...
const DB_NAME='speedfeet-v2',DB_VERSION=2;
...
async function beginPreparation(){
  ...
  $('plannedTests').value='';$('sessionGoal').selectedIndex=0;$('forecastWind').value='';$('forecastDir').value='';$('forecastSea').value='';$('boatConfig').value='';
...
$('startNav').onclick=async()=>{
  ...
  await put('sessions',{id:currentNavId,type:'navigation',title:'Navigation du '+navStart.toLocaleString('fr-FR'),startedAt:navStart.toISOString(),goal:$('sessionGoal').value,plannedTests:$('plannedTests').value,forecast:{wind:Number($('forecastWind').value)||null,dir:Number($('forecastDir').value)||null,sea:$('forecastSea').value.trim()},boat:{main:$('boatMain').value,spi:$('boatSpi').value,config:$('boatConfig').value.trim()},status:'active',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});
...
$('markBtn').onclick=()=>{selectedMarkerType='';$('markerComment').value='';$('speechState').textContent='';$('windFields').classList.add('hidden');$('trimFields').classList.add('hidden');document.querySelectorAll('[data-marker-type]').forEach(b=>b.classList.remove('selected'));$('markerModal').classList.remove('hidden');confirmationBeep()};
$('dictateMarker').onclick=()=>startDictation('markerComment','speechState');
document.querySelectorAll('[data-marker-type]').forEach(b=>b.onclick=()=>{selectedMarkerType=b.dataset.markerType;document.querySelectorAll('[data-marker-type]').forEach(x=>x.classList.toggle('selected',x===b));$('windFields').classList.toggle('hidden',selectedMarkerType!=='wind');$('trimFields').classList.toggle('hidden',selectedMarkerType!=='trim')});
...
$('saveMarker').onclick=async()=>{
  ...
  const type=selectedMarkerType||'note',typeCount=existing.filter(x=>x.type===type).length+1;
  const labels={tack:'Virement',gybe:'Empannage',wind:'Mesure de vent',trim:'Changement de réglage','spi-hoist':'Envoi du spi','spi-drop':'Affalage du spi',note:'Marqueur'};
  const label=(labels[type]||'Marqueur')+(['tack','gybe'].includes(type)?` ${typeCount}`:'');
  const note=$('markerComment').value.trim(),data={};
  if(type==='wind'){data.windSpeed=Number($('eventWindSpeed').value)||null;data.windDir=Number($('eventWindDir').value)||null}
  if(type==='trim'){data.trim={mainTraveler:$('trimMainTraveler').value,jibTraveler:$('trimJibTraveler').value,mastRotation:$('trimMastRotation').value,cunningham:$('trimCunningham').value,outhaul:$('trimOuthaul').value,mainSheet:$('trimMainSheet').value};data.startTest=$('startTrimTest').checked}
  await put('markers',{id:uuid(),sessionId:currentNavId,time:now.toISOString(),type,number:typeCount,note,data,position:lastPos,createdAt:now.toISOString()});
...
$('analyzeBtn').onclick=analyzeCurrent;$('sessionSelect').onchange=analyzeCurrent;$('attachSession').onchange=()=>{$('autoSync').disabled=!$('attachSession').value;$('syncState').textContent=$('attachSession').value?'Prêt à synchroniser les événements de cette navigation.':'Sélectionne une navigation enregistrée pour synchroniser ses événements avec la trace.'};
function analyzeCurrent(){current=allSessions[Number($('sessionSelect').value)||0];const st=calcStats(current),wind=Number($('windDir').value)||0;maneuvers=detectManeuvers(current,wind);$('results').classList.remove('hidden');$('sDate').textContent=current[0].time?current[0].time.toLocaleDateString('fr-FR'):'—';$('sDuration').textContent=fmtDuration(st.duration);$('sDistance').textContent=st.distance.toFixed(2)+' nm';$('sMax').textContent=st.max.toFixed(2)+' nd';$('sAvg').textContent=st.avg.toFixed(2)+' nd';$('sTacks').textContent=maneuvers.filter(x=>x.type==='tack').length;$('sGybes').textContent=maneuvers.filter(x=>x.type==='gybe').length;$('sPoints').textContent=current.length.toLocaleString('fr-FR');$('timeline').max=current.length-1;drawMap();updateReplay(0);drawPolar(current,wind);renderTrimTests([])}
function drawMap(){if(map)map.remove();map=L.map('map',{preferCanvas:true});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);const ll=current.map(p=>[p.lat,p.lon]);map.fitBounds(ll,{padding:[20,20]});for(let i=1;i<current.length;i++)L.polyline([[current[i-1].lat,current[i-1].lon],[current[i].lat,current[i].lon]],{color:colorForSpeed((current[i-1].speed+current[i].speed)/2),weight:4,opacity:.9}).addTo(map);(window.syncedEvents||[]).forEach(e=>{if(e.syncIndex==null)return;const p=current[e.syncIndex],label={wind:'Vent',trim:'Réglage',tack:'Virement',gybe:'Empannage','spi-hoist':'Envoi spi','spi-drop':'Affalage spi'}[e.type]||'Événement';const icon=L.divIcon({className:'',html:`<div class="event-icon">${label[0]}</div>`,iconSize:[28,28],iconAnchor:[14,14]});L.marker([p.lat,p.lon],{icon}).bindPopup(`<b>${label}</b><br>${new Date(e.time).toLocaleTimeString('fr-FR')}<br>${esc(e.note||'')}`).addTo(map)});maneuvers.forEach(m=>{const p=m.type==='tack'?'V':'E',point=current[m.index],label=m.type==='tack'?'Virement':'Empannage',details=`<b>${label} ${m.number}</b><br>Heure : ${point?.time?point.time.toLocaleTimeString('fr-FR'):'—'}<br>Vitesse : ${Number(point?.speed||0).toFixed(1)} nd<br>Cap : ${Math.round(point?.heading||0)}°`;const icon=L.divIcon({className:'',html:`<div class="maneuver-icon maneuver-${m.type}">${p}${m.number}</div>`,iconSize:[28,28],iconAnchor:[14,14]});L.marker([m.lat,m.lon],{icon}).bindTooltip(details,{sticky:true}).bindPopup(details).addTo(map)});trail=L.polyline([],{color:'#fff',weight:5}).addTo(map);replayMarker=L.circleMarker(ll[0],{radius:9,color:'#fff',weight:3,fillColor:'#111',fillOpacity:1}).addTo(map);setTimeout(()=>map.invalidateSize(),100)}
...
async function saveAnalysisToSession(attachId=''){if(!current.length)return null;const st=calcStats(current),date=current[0].time?.toISOString()||new Date().toISOString();let s=attachId?await getOne('sessions',attachId):null;if(!s)s={id:uuid(),type:'navigation',title:'Navigation analysée du '+new Date(date).toLocaleString('fr-FR'),startedAt:date,createdAt:new Date().toISOString()};Object.assign(s,{fileName:loadedFileName,date,syncOffset:window.syncOffset||0,syncedEvents:(window.syncedEvents||[]).map(e=>({id:e.id,type:e.type,time:e.time,syncIndex:e.syncIndex})),startedAt:s.startedAt||date,endedAt:current.at(-1).time?.toISOString(),distance:st.distance,duration:st.duration,maxSpeed:st.max,avgSpeed:st.avg,windDir:Number($('windDir').value)||0,windSpeed:Number($('windSpeed').value)||0,points:current.map(p=>({...p,time:p.time?.toISOString?.()||p.time})),maneuvers,status:'finished',updatedAt:new Date().toISOString()});await put('sessions',s);await refreshDashboard();await populateAttachSessions();return s}
async function loadStoredAnalysis(s){current=(s.points||[]).map(p=>({...p,time:p.time?new Date(p.time):null}));if(!current.length)return;loadedFileName=s.fileName||'';$('windDir').value=s.windDir||0;$('windSpeed').value=s.windSpeed||0;maneuvers=s.maneuvers?.length?s.maneuvers:detectManeuvers(current,Number($('windDir').value)||0);go('analyze');$('results').classList.remove('hidden');const st=calcStats(current);$('sDate').textContent=current[0].time?current[0].time.toLocaleDateString('fr-FR'):'—';$('sDuration').textContent=fmtDuration(st.duration);$('sDistance').textContent=st.distance.toFixed(2)+' nm';$('sMax').textContent=st.max.toFixed(2)+' nd';$('sAvg').textContent=st.avg.toFixed(2)+' nd';$('sTacks').textContent=maneuvers.filter(x=>x.type==='tack').length;$('sGybes').textContent=maneuvers.filter(x=>x.type==='gybe').length;$('sPoints').textContent=current.length.toLocaleString('fr-FR');$('timeline').max=current.length-1;drawMap();updateReplay(0);drawPolar(current,Number($('windDir').value)||0);renderTrimTests([]);$('attachSession').value=s.id;$('autoSync').disabled=false;$('status').textContent='Analyse chargée depuis le carnet'}
$('saveSession').onclick=async()=>{await saveAnalysisToSession($('attachSession').value);toast('Analyse enregistrée dans le carnet')};

function nearestPointIndex(time,offsetSec=0){if(!current.length||!time)return-1;const target=new Date(time).getTime()+offsetSec*1000;let lo=0,hi=current.length-1;while(lo<hi){const m=(lo+hi)>>1;if((current[m].time?.getTime()||0)<target)lo=m+1;else hi=m}if(lo>0&&Math.abs((current[lo-1].time?.getTime()||0)-target)<Math.abs((current[lo].time?.getTime()||0)-target))lo--;return lo}
async function synchronizeEvents(){const sid=$('attachSession').value;if(!sid||!current.length)return;const sess=await getOne('sessions',sid),events=(await all('markers')).filter(m=>m.sessionId===sid).sort((a,b)=>a.time.localeCompare(b.time));if(!sess||!events.length){$('syncState').textContent='Aucun événement enregistré dans cette navigation.';return}let auto=0;if(sess.startedAt&&current[0].time)auto=(current[0].time-new Date(sess.startedAt))/1000;const manual=Number($('manualOffset').value)||0,offset=auto+manual;events.forEach(e=>{const i=nearestPointIndex(e.time,offset);if(i>=0){e.syncIndex=i;e.syncLat=current[i].lat;e.syncLon=current[i].lon;e.syncSpeed=current[i].speed;e.syncHeading=current[i].heading}});window.syncedEvents=events;window.syncOffset=offset;$('appliedOffset').innerHTML=`<b style="font-size:18px">${offset>=0?'+':''}${Math.round(offset)} s</b>`;$('syncState').textContent=`${events.length} événement${events.length>1?'s':''} synchronisé${events.length>1?'s':''}. Décalage automatique ${Math.round(auto)} s, correction manuelle ${manual} s.`;drawMap();renderTrimTests(buildTrimTests(events));toast('Synchronisation terminée')}
$('autoSync').onclick=synchronizeEvents;
function drawPolar(points,wind){const c=$('polarCanvas');if(!c)return;const ctx=c.getContext('2d'),W=c.width,H=c.height,cx=W/2,cy=H-28,R=Math.min(W*.43,H*.82),bins=[];for(let a=0;a<=180;a+=10){const vals=points.filter(p=>p.speed>.5&&Math.abs(angDiff(p.heading,wind)-a)<=5).map(p=>p.speed).sort((x,y)=>x-y);const top=vals.slice(Math.floor(vals.length*.7));bins.push({a,v:top.length?top[Math.floor(top.length/2)]:0})}const max=Math.max(6,...bins.map(x=>x.v));ctx.clearRect(0,0,W,H);ctx.strokeStyle='#233955';ctx.fillStyle='#9fb0c3';ctx.font='12px sans-serif';ctx.textAlign='center';for(let k=1;k<=4;k++){ctx.beginPath();ctx.arc(cx,cy,R*k/4,Math.PI,2*Math.PI);ctx.stroke();ctx.fillText((max*k/4).toFixed(1),cx,cy-R*k/4-4)}for(let a=0;a<=180;a+=30){const r=a*Math.PI/180,x=cx-R*Math.cos(r),y=cy-R*Math.sin(r);ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(x,y);ctx.stroke();ctx.fillText(a+'°',cx-(R+16)*Math.cos(r),cy-(R+16)*Math.sin(r))}ctx.strokeStyle='#42a5ff';ctx.lineWidth=3;ctx.beginPath();bins.forEach((b,i)=>{const r=b.a*Math.PI/180,rr=R*b.v/max,x=cx-rr*Math.cos(r),y=cy-rr*Math.sin(r);i?ctx.lineTo(x,y):ctx.moveTo(x,y)});ctx.stroke();ctx.fillStyle='#42a5ff';bins.forEach(b=>{if(!b.v)return;const r=b.a*Math.PI/180,rr=R*b.v/max,x=cx-rr*Math.cos(r),y=cy-rr*Math.sin(r);ctx.beginPath();ctx.arc(x,y,3.5,0,Math.PI*2);ctx.fill()})}
function buildTrimTests(events){const starts=events.filter(e=>e.type==='trim'&&e.data?.startTest&&Number.isInteger(e.syncIndex));return starts.map((e,i)=>{const end=starts[i+1]?.syncIndex??Math.min(current.length-1,e.syncIndex+600),slice=current.slice(e.syncIndex,end).filter(p=>p.speed>.5),avgSpeed=slice.length?avg(slice.map(p=>p.speed)):0;return{id:e.id,start:e.syncIndex,end,avgSpeed,trim:e.data.trim,note:e.note,status:'pending'}})}
function renderTrimTests(tests){const el=$('trimTestResults');if(!el)return;if(!tests.length){el.innerHTML='<div class="muted">Aucun test de réglage synchronisé.</div>';return}const best=Math.max(...tests.map(t=>t.avgSpeed));el.innerHTML=tests.map((t,i)=>{const d=t.avgSpeed-best,cls=d>=-.05?'positive':'negative';return `<div class="testrow"><b>Test ${i+1}</b><div class="gain ${cls}">${t.avgSpeed.toFixed(2)} nd ${d>=-.05?'— meilleur':'('+d.toFixed(2)+' nd)'}</div><div class="muted small">${esc(Object.entries(t.trim||{}).filter(([,v])=>v).map(([k,v])=>k+': '+v).join(' · ')||t.note||'Réglage non détaillé')}</div><div class="row" style="margin-top:8px"><button class="iconbtn" data-test-status="validated">Valider</button><button class="iconbtn" data-test-status="retest">Retester</button><button class="iconbtn" data-test-status="rejected">Rejeter</button></div></div>`}).join('')}
document.addEventListener('click',e=>{if(e.target.dataset.testStatus)toast(e.target.dataset.testStatus==='validated'?'Test validé dans la base personnelle':e.target.dataset.testStatus==='retest'?'Test conservé à retester':'Test rejeté')});
async function clearStore(name){for(const row of await all(name))await del(name,row.id)}
$('deleteOneNav').onclick=async()=>{const ss=(await all('sessions')).filter(s=>s.status!=='active').sort((a,b)=>(b.startedAt||'').localeCompare(a.startedAt||''));if(!ss.length)return toast('Aucune navigation à supprimer');const names=ss.map((s,i)=>`${i+1}. ${s.title}`).join('\n'),n=Number(prompt('Numéro de la navigation à supprimer :\n'+names));if(!n||!ss[n-1])return;const s=ss[n-1];if(!confirm(`Supprimer « ${s.title} » et ses événements ?`))return;for(const m of (await all('markers')).filter(m=>m.sessionId===s.id))await del('markers',m.id);for(const t of (await all('tasks')).filter(t=>t.sessionId===s.id))await del('tasks',t.id);await del('sessions',s.id);refreshDashboard();toast('Navigation supprimée')};
$('deleteAllNav').onclick=async()=>{if(!confirm('Supprimer toutes les navigations, événements et tâches liés ?'))return;await clearStore('sessions');await clearStore('markers');await clearStore('tasks');refreshDashboard();toast('Toutes les navigations ont été supprimées')};
$('resetDatabase').onclick=async()=>{if(!confirm('Réinitialiser toute la base locale ? Cette action est irréversible.'))return;for(const n of ['settings','sessions','markers','tasks'])await clearStore(n);location.reload()};
