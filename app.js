...
$('markBtn').onclick=()=>{selectedMarkerType='';$('markerComment').value='';document.querySelectorAll('[data-marker-type]').forEach(b=>b.classList.remove('selected'));$('markerModal').classList.remove('hidden')};
...
$('saveMarker').onclick=async()=>{
  ...
  $('markerModal').classList.add('hidden');toast('Marqueur enregistré');
};
...
async function openSession(id){
  ...
  $('sessionDetail').innerHTML=`<div class="card">
    ...
    ${s.points?.length?'<div class="detailrow"><b>Analyse VCC</b><div class="muted">Trace GPS et statistiques enregistrées. Ouvre l’onglet Analyser pour une lecture détaillée.</div></div>':''}
...
async function refreshDashboard(){
  ...
  $('recentSessions').innerHTML=recent.length?recent.map(s=>`<div class="taskitem sessioncard" data-go="logbook"><div class="tasktext"><b>${esc(s.title||'Navigation enregistrée')}</b><div class="muted">${formatDate(s.startedAt||s.date)}</div></div></div>`).join(''):'Aucune sortie enregistrée.';
...
function drawMap(){if(map)map.remove();map=L.map('map',{preferCanvas:true});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);const ll=current.map(p=>[p.lat,p.lon]);map.fitBounds(ll,{padding:[20,20]});for(let i=1;i<current.length;i++)L.polyline([[current[i-1].lat,current[i-1].lon],[current[i].lat,current[i].lon]],{color:colorForSpeed((current[i-1].speed+current[i].speed)/2),weight:4,opacity:.9}).addTo(map);maneuvers.forEach(m=>{const p=m.type==='tack'?'V':'E',icon=L.divIcon({className:'',html:`<div class="maneuver-icon maneuver-${m.type}">${p}${m.number}</div>`,iconSize:[28,28],iconAnchor:[14,14]});L.marker([m.lat,m.lon],{icon}).bindTooltip(m.type==='tack'?'Virement':'Empannage').addTo(map)});trail=L.polyline([],{color:'#fff',weight:5}).addTo(map);replayMarker=L.circleMarker(ll[0],{radius:9,color:'#fff',weight:3,fillColor:'#111',fillOpacity:1}).addTo(map);setTimeout(()=>map.invalidateSize(),100)}
...
$('saveSession').onclick=async()=>{if(!current.length)return;const st=calcStats(current),date=current[0].time?.toISOString()||new Date().toISOString(),attachId=$('attachSession').value;let s=attachId?await getOne('sessions',attachId):null;if(!s)s={id:uuid(),type:'navigation',title:'Navigation analysée du '+new Date(date).toLocaleString('fr-FR'),startedAt:date,createdAt:new Date().toISOString()};Object.assign(s,{fileName:loadedFileName,date,startedAt:s.startedAt||date,endedAt:current.at(-1).time?.toISOString(),distance:st.distance,duration:st.duration,maxSpeed:st.max,avgSpeed:st.avg,windDir:Number($('windDir').value)||0,windSpeed:Number($('windSpeed').value)||0,points:current,maneuvers,status:'finished',updatedAt:new Date().toISOString()});await put('sessions',s);toast('Analyse enregistrée dans le carnet');refreshDashboard();populateAttachSessions()};
